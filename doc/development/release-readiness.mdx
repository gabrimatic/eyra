...
- GitHub Pages deploys are explicitly guarded with `EYRA_DEPLOY_PAGES=true`; private repos should leave the variable unset unless Pages is configured.
...
- Menu bar install reality: `eyra menu --json` reports source, package-resource, app-bundle, or unavailable mode; docs use `eyra open` as the reliable installed-user control path until a packaged `.app` bundle is shipped.
