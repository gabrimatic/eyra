...
Open the installed-user control UI:
...
Check the menu bar developer preview:

```bash
eyra menu --json
```

The menu bar preview requires Swift/Xcode command-line tools until a packaged `.app` bundle ships. If it is unavailable, keep using `eyra open`.
...
Run the menu bar developer preview from source:
