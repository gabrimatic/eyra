...
```bash
EYRA_VERSION=v4.3.1 EYRA_INSTALL_DIR="$HOME/.local/share/eyra/app" ./install.sh
```
...
The checked-in formula installs from the `main` branch and reports `4.3.1`. The formula remains tap preparation until it is switched to a tagged release URL and pinned sha256. Treat it as tap preparation, not a stable public formula.
...
The guided installer and `setup.sh` try this for you when npm is available. If Node.js is missing, install it from Homebrew or from nodejs.org, then rerun `eyra doctor`.
