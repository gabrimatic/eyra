...
```bash
EYRA_VERSION=v4.3.3 EYRA_INSTALL_DIR="$HOME/.local/share/eyra/app" ./install.sh
```
...
The formula installs the latest tagged GitHub release asset with a pinned checksum.
...
The guided installer and `setup.sh` try the npm package first, then fall back to the GitHub release tarball if npm authentication or registry access is unavailable. If Node.js is missing, install it from Homebrew or from nodejs.org, then rerun `eyra doctor`.
