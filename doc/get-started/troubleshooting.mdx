...
If the status says `memory file error`, the local JSON file may be corrupt or unreadable. The path is shown by `eyra memory status`. Move the file aside if you want Eyra to start a fresh memory file.
...
With that setting, `remember` commands still save, but ordinary auto-save candidates are skipped.
