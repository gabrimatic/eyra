...
- The Homebrew formula runs `eyra version`, `eyra paths --json`, and `eyra doctor --json`; the formula test uses mock, typed-only mode so it does not require a real configured backend. Until the formula is switched from the main-branch scaffold to a tagged asset and pinned sha256, treat it as custom-tap preparation rather than a stable public formula.
