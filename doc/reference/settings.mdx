...
| `MEMORY_AUTO_SAVE_ENABLED` | `true` | Allow explicit memory saves |
...
| `MEMORY_WRITE_REQUIRE_CONFIRMATION` | `false` | Reserved confirmation gate for future stricter memory writes |
...
Memory is local by default. Eyra stores compact key/value facts and refuses raw conversations, secrets, screenshots, clipboard dumps, stack traces, PDF text, and long tool output.
