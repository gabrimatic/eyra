...
`install.sh` installs a release archive into a managed directory and runs non-interactive setup. For `v4.2.0rc1`, use authenticated GitHub access while the repository is private.
...
The checked-in formula installs from the `master` branch and reports `4.2.0rc1`. Even when release-candidate assets exist, this formula remains private-beta tap preparation until it is switched to a tagged release URL and pinned sha256. Treat it as tap preparation, not a stable public formula.
...
- Runs `eyra version`, `eyra paths --json`, and `eyra doctor --json` in the formula test. The formula test expects a real configured backend; it is not a stable public formula yet.
