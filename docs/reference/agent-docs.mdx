...
This repo keeps the public docs site in `docs/`, with source pages in MDX, explicit frontmatter, and navigation in `docs/docs.json`.
...
The repository also publishes the Mintlify export to GitHub Pages from the `docs/` directory. On every push to `master` that changes docs, `.github/workflows/docs-pages.yml` runs `mint export`, patches root-relative paths for GitHub project Pages, uploads the static artifact, and deploys it.

The public GitHub Pages URL is:

```text
https://gabrimatic.github.io/eyra/
```
...
This branch creates the Mintlify source site. Hosted AI-native artifacts such as live `llms.txt`, `llms-full.txt`, `skill.md`, and the documentation MCP server are available after deployment through Mintlify's hosted layer.
