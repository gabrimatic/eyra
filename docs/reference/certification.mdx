...
- The Homebrew formula has no placeholder checksum and runs `eyra version`, `eyra paths --json`, and `eyra doctor --json`; the formula test expects a real configured backend.
