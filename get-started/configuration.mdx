...
`.env` is the single source of truth. `Settings.load_from_env()` reads every setting with defaults, validates booleans and numeric ranges, and feeds the runtime, Web UI, routing, tools, and voice stack.
