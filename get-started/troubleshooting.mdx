...
Start with `/status`, `/capabilities`, and `/context`. They show the current runtime state, enabled control surfaces, privacy boundary, recent jobs, and recent local changes.
